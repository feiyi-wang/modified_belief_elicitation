$(document).ready(function () {
    ...
    function chart(chartIndex, params) {
        ...
        function drawChart(parameters) {
            ...
            new Highcharts.Chart({
                ...
                chart: {
                    ...
                    events: {
                        click: function (e) {
                            let xValue = Math.round(e.xAxis[0].value / step) * step;
                            let existingPoint = this.series[0].data.find(point => point.x === xValue);

                            if (!existingPoint) {
                                add_point(this.series[0], xValue, e.yAxis[0].value);
                            } else {
                                console.log("Point already exists at x:", xValue);
                            }
                        }
                    }
                },
                ...
                xAxis: [{
                    ...
                    tickPositions: Array.from({ length: 9 }, (_, i) => js_vars.min + i * ((1 - js_vars.min) / (9 - 1))),
                    labels: {
                        formatter: function () {
        ...
        function add_point(series, x, y) {
            let existingPoint = series.data.find(point => point.x === x);

            if (!existingPoint) {
                series.addPoint([x, y], true, false, false);
                console.log("Added point at x:", x, "y:", y);
            } else {
