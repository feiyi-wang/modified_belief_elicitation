...
$(document).ready(function() {
    ...
    chart(index, $(this).data());
    $(this).before('<div style="text-align: right;"><button class="undo btn btn-warning mx-1" disabled type="button">Undo</button><button class="reset btn btn-danger mx-1" disabled type="button">Reset</button></div>');
    $(this).after('<div class="mt-2" style="text-align: center;margin-left:50px"><a class="SubmitDistribution btn btn-success" disabled>Submit the distribution</a></div>');
    });
});
    function chart(chartindex, params){
        ...
        let initdataspline = tickPositions().map(x => [x, 0]);
        let initsaved = [
    ...
    if (mode === "curve" && x >= (min - step / 2) && x <= (max + step / 2)) {
        ...
        let pointExists = serie.data.some(point => point.x === x);

        if (pointExists) {
            console.log("Point already exists at x =", x);
        ...
        // Update bins after adding the point
        ...
        function drawChart(parameters) {
            ...
            xAxis: [{
                    ...
                    labels: {
                          ...
                          let labels = ['Worst <br>possible', 'Very <br>poor', 'Poor', 'Somewhat <br>poor', 'Average', 'Somewhat <br>good', 'Good', 'Very <br>good', 'Best <br>possible'];
                          
                          let index = this.value / step;
                          
                          return labels[Math.round(index)] || '';
